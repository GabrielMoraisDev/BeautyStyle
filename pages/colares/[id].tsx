  ...
  useEffect(() => {
    const fetchOrders = async () => {
      try {
        ...
        const response = await fetch('http://localhost/Beautystyle/pedidos.php?id_user=' + userId);
        if (!response.ok) {
  ...
  useEffect(() => {
    if (id) {
      const fetchProduct = async () => {
        try {
          const response = await fetch(`http://localhost/BeautyStyle/products.php?categoria=colares`);
          if (!response.ok) {
  ...
  const addToCart = () => {
    ...
    if (product) {
      ...
      const productWithDiscount = {
        ...
        imagem: `/img/products/colares/${product.id}.webp`,
        quantity: qnt,
